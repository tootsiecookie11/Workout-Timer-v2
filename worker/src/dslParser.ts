/**
 * Server-side DSL Parser + Evaluator (PRD §5.9).
 *
 * Mirrors app/src/engine/dslParser.ts — kept in the worker so the
 * /api/parse-dsl endpoint can validate and compile DSL conditions
 * without shipping untrusted strings to the client for evaluation.
 *
 * Grammar:
 *   expr       = or_expr
...
import type { ASTNode, Operand, CompareOp, EvalContext, Span } from './dslTypes';
...
function resolveOperand(op: Operand, ctx: EvalContext): ScalarValue {
  switch (op.kind) {
    ...
    case 'var': {
      const val = (ctx as unknown as Record<string, unknown>)[op.name];
      if (val === undefined) {
